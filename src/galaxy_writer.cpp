		...
		for (auto &subhalo: halo->all_subhalos()){
			...
			for (auto &galaxy: subhalo->galaxies){
				...
				if(galaxy.galaxy_type == Galaxy::CENTRAL || galaxy.galaxy_type == Galaxy::TYPE1){
					...
					rvir_subhalo.push_back(galaxy.msubhalo_type2); // just a placeholder; mas as virial mass
...
#define REPORT(x) total += report_vsize(x, os, #x)
	...
	comment = "Virial radius at infall of the host halo in which this galaxy reside when it was last central [Msun/h]";
